  ...
  "node_modules/use-sync-external-store/cjs/use-sync-external-store-shim.development.js"(exports) {
    ...
    if (true) {
      (function() {
        ...
        var useState5 = React.useState, useEffect6 = React.useEffect, useLayoutEffect = React.useLayoutEffect, useDebugValue2 = React.useDebugValue;
  ...
  "node_modules/use-sync-external-store/cjs/use-sync-external-store-shim/with-selector.development.js"(exports) {
    ...
    if (true) {
      (function() {
        ...
        var useRef2 = React.useRef, useEffect6 = React.useEffect, useMemo2 = React.useMemo, useDebugValue2 = React.useDebugValue;
...
// app/services/nostr/hooks/store/index.ts
...
// app/services/nostr/hooks/utils/index.ts
...
var mergeFilters = (filters) => {
  return import_lodash.default.chain(filters).filter((f) => !isFilterCorrupted(f)).groupBy((f) => {
    ...
    (groupedFilters) => import_lodash.default.mergeWith(
      {},
      ...groupedFilters,
      (objValue, srcValue) => import_lodash.default.isArray(objValue) ? import_lodash.default.uniq(objValue.concat(srcValue)) : void 0
    )
  ).value();
};
...
var useNostrStore = create()((set, get) => ({
  ...
  handleCacheRefresh: (subId, config) => {
    ...
    if (get().isBatching === false) {
      setTimeout(get().processQueue, ((_a = config.options) == null ? void 0 : _a.batchingInterval) || 500);
  ...
  handleNewSub: ({ filters, relays, options }, subId) => {
    ...
    if (options == null ? void 0 : options.invalidate) {
    ...
    if (alreadyHasEvents) {
      if (options == null ? void 0 : options.cacheRefresh) {
    ...
    if (get().isBatching === false) {
      setTimeout(get().processQueue, (options == null ? void 0 : options.batchingInterval) || 500);
      get().setIsBatching(true);
    }
  },
  handlePoolSub: (queueMap) => {
    ...
    queueMap.forEach((config) => {
      ...
      if (((_a = config.options) == null ? void 0 : _a.closeAfterEose) === false) {
  ...
  loadMore: (subId) => {
    var _a, _b;
    const config = (_a = get().subMap.get(subId)) == null ? void 0 : _a.config;
    if (!config)
      return;
    ...
    const oldestEventTimestamp = import_lodash2.default.min(
    ...
    get().insertIntoQueue(config, subId);
    if (get().isBatching === false) {
      setTimeout(get().processQueue, ((_b = config.options) == null ? void 0 : _b.batchingInterval) || 500);
      get().setIsBatching(true);
    }
  },
  processQueue: () => {
  ...
  setEoseByFilters: (filters, eose) => set((store) => {
    store.subMap.forEach(
      (sub) => import_lodash2.default.isEqual(sub.config.filters, filters) && (sub.eose = eose)
    );
    return { subMap: store.subMap };
  }),
...
// app/services/nostr/hooks/hooks/useSubscribe.ts
var import_react2 = __toESM(require_react());
var useSubscribe = ({ filters, relays, options }) => {
  const subId = (0, import_react2.useRef)(generateSubId());
  const shouldCreateSub = (0, import_react2.useRef)(true);
  const handleSub = useNostrStore(
    (0, import_react2.useCallback)((store) => store.handleNewSub, [])
  );
  const handleUnSub = useNostrStore((0, import_react2.useCallback)((store) => store.unSub, []));
  const handleInvalidate = useNostrStore(
    (0, import_react2.useCallback)((store) => store.handleInvalidate, [])
  );
  const loadMore = useNostrStore((0, import_react2.useCallback)((store) => store.loadMore, []));
  const sub = useNostrStore(
    (0, import_react2.useCallback)((store) => store.subMap.get(subId.current), [subId.current])
  );
  ...
  (0, import_react2.useEffect)(() => {
    ...
    shouldCreateSub.current = true;
  }, [options == null ? void 0 : options.enabled, options == null ? void 0 : options.invalidate]);
  (0, import_react2.useEffect)(() => {
  ...
  return {
    events,
    eose: (sub == null ? void 0 : sub.eose) || false,
    invalidate: () => handleInvalidate(subId.current, { filters, relays, options }),
    loadMore: () => loadMore(subId.current)
  };
};
var useSubscribe_default = useSubscribe;

// app/services/nostr/subscribe.ts
...
// node_modules/nostr-relaypool/lib/nostr-relaypool.esm.js
...
var RelayC = class {
  ...
  async connect() {
    if (this.ws?.readyState && this.ws.readyState < 2)
      return;
...
var RelayPool = class {
  ...
  async #getRelaysAndSubscribe(filters, onEvent, maxDelayms, onEose, options = {}) {
    ...
    for (const author of allAuthors) {
      promises.push(
        this.writeRelays?.get(author).then((event) => parseJSON(event?.content)).catch(() => options?.defaultRelays || [])
      );
...
// app/services/state.ts
var import_lodash3 = __toESM(require_lodash());
var import_react3 = __toESM(require_react());
var notInLocalStorage = /* @__PURE__ */ new Set();
var LEAF = 1;
var BRANCH = 2;
var Node = class {
  /** */
  constructor(path = "localState", parent = null) {
    this.children = /* @__PURE__ */ new Map();
    this.on_subscriptions = /* @__PURE__ */ new Map();
    this.list_subscriptions = /* @__PURE__ */ new Map();
    this.value = void 0;
    this.counter = 0;
    this.loaded = false;
    this.saveLocalStorage = import_lodash3.default.throttle(async () => {
      if (!this.loaded) {
    ...
    this.loadLocalStorage = import_lodash3.default.throttle(async () => {
      ...
      if (result === null || result === void 0) {
        ...
        try {
          ...
          if ((result == null ? void 0 : result.type) && (result == null ? void 0 : result.value)) {
            if (result.type === BRANCH) {
              const newResult = {};
              await Promise.all(
                result.value.map(async (key) => {
    ...
    this.doCallbacks = import_lodash3.default.throttle(() => {
      ...
      if (this.parent) {
        for (const [id, callback] of this.parent.on_subscriptions) {
          const unsub = () => {
            var _a;
            return (_a = this.parent) == null ? void 0 : _a.on_subscriptions.delete(id);
          };
        ...
        for (const [id, callback] of this.parent.list_subscriptions) {
          const unsub = () => {
            var _a;
            return (_a = this.parent) == null ? void 0 : _a.list_subscriptions.delete(id);
          };
          this.once(callback, unsub, false);
        }
      }
  ...
  set(value) {
    ...
    if (typeof value === "object" && value !== null) {
      ...
      import_lodash3.default.defer(() => this.doCallbacks(), 100);
      return;
    }
    this.children = /* @__PURE__ */ new Map();
    this.value = value;
   ...
   * @returns {Promise<*>}
   */
  async once(callback, unsub, returnIfUndefined = true) {
    let result;
    if (this.children.size) {
      result = {};
      await Promise.all(
        Array.from(this.children.keys()).map(async (key) => {
          result[key] = await this.get(key).once(void 0, unsub);
        })
      );
    } else if (this.value !== void 0) {
      result = this.value;
    ...
    if (result !== void 0 || returnIfUndefined) {
      callback && callback(
        result,
...
var localState = new Node();
var state_default = localState;

// app/services/nostr/subscribe.ts
var globalRelayPool = null;
var DEFAULT_RELAYS = [
  "wss://relay.damus.io",
  "wss://relay.snort.social",
  "wss://relay.gems.xyz"
];
var useRelayPoolSubscribe = (ops) => {
  const { relays, filters, options, maxDelayms, onEose, enabled } = ops;
  const [events, setEvents] = (0, import_react4.useState)([]);
  const [eose, setEose] = (0, import_react4.useState)(false);
  ...
  const [relayPool] = (0, import_react4.useState)(initalRelayPool);
  const [unsubscribe, setUnsubscribe] = (0, import_react4.useState)(() => () => ({}));
...
var useRelayPool = false;
state_default.get("useRelayPool").on((value) => {
  useRelayPool = !!value;
});
function useSubscribe2(opts) {
  const relayPoolSubscribe = useRelayPoolSubscribe({
    ...opts,
    enabled: useRelayPool
  });
  const nostrHooksSubscribe = useSubscribe_default({
    ...opts,
    enabled: !useRelayPool
  });
...
// app/services/nostr/profiles/content.ts
var import_react7 = __toESM(require_react());

// app/services/nostr/profiles/hex.ts
var import_react5 = __toESM(require_react());

// app/utils/nostr-profile.ts
async function getProfileHex(profileAddress) {
  ...
  if (profileAddressType === "nip05") {
    ...
    const profile = await nip05_exports.queryProfile(profileAddress);
    if (!profile || !profile.pubkey)
      throw new Error("profile not found");
    return profile.pubkey;
  } else if (profileAddressType === "nip19") {
    const { data } = nip19_exports.decode(profileAddress);
    if (!data)
      throw new Error("profile not found");
...
var getProfileAddressType = (profileAddress) => {
...
// app/services/nostr/profiles/hex.ts
var useProfileHex = (profileAddress) => {
  const [profileHex, setProfileHex] = (0, import_react5.useState)("");
...
var hex_default = useProfileHex;

// app/services/store/pool.ts
var _pool = new SimplePool();
var createPoolSlice = (_set, get) => ({
  pool: {
    simplePool: _pool,
    sub: (filters) => _pool.sub(get().relays, filters),
    list: (filters) => _pool.list(get().relays, filters),
    publish: (event) => _pool.publish(get().relays, event)
  },
  relays: [
    "wss://relay.damus.io",
    "wss://relay.snort.social",
...
var pool_default = createPoolSlice;

// app/services/store/auth.ts
var getUserRelays = (_pub) => {
};
var getLocalStorage = () => {
...
var createAuthSlice = (set) => ({
  auth: {
    user: {
      data: getLocalStorage()
    },
    loginWithPublicKey: (publicKey) => {
...
var auth_default = createAuthSlice;

// app/services/store/index.ts
var useStore2 = create()((...a) => ({
  ...pool_default(...a),
  ...auth_default(...a)
}));
var store_default = useStore2;

// app/services/nostr/profiles/metadata.ts
var import_react6 = __toESM(require_react());
var useProfileMetadata = (profileAddress) => {
  ...
  const latestMetadataEvent = (0, import_react6.useMemo)(() => {
    return metadataEvents == null ? void 0 : metadataEvents.reduce((prev, curr) => {
      if (!prev)
        return curr;
...
var metadata_default = useProfileMetadata;

// app/services/nostr/profiles/content.ts
var useProfileContent = (profileAddress) => {
  const hex = hex_default(profileAddress);
  const {
    isFetchingMetadata,
    isMetadataEmpty,
    metadataEose,
    latestMetadataEvent,
    metadata
  } = metadata_default(hex);
  const [author, setAuthor] = (0, import_react7.useState)({
    about: "",
    banner: "",
    following: [],
    id: "",
  ...
  const [npub, setNpub] = (0, import_react7.useState)(void 0);
  (0, import_react7.useEffect)(() => {
    ...
    setNpub(!!profileAddress && nip19_exports.npubEncode(hex) || void 0);
  }, [
    hex,
    latestMetadataEvent == null ? void 0 : latestMetadataEvent.content,
    metadata.about,
    metadata.banner,
...
var content_default = useProfileContent;

export {
  require_lodash,
  create,
  useSubscribe2 as useSubscribe,
  hex_default,
  store_default,
  content_default
};
...
use-sync-external-store/cjs/use-sync-external-store-shim/with-selector.development.js:
   ...
   *)
*/
//# sourceMappingURL=/build/_shared/chunk-VF2ZEHT7.js.map
